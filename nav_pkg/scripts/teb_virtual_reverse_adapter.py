...
class TebVirtualReverseAdapter:
    def __init__(self):
        self.external_action_name = rospy.get_param("~external_action_name", "move_base")
        self.internal_action_name = rospy.get_param("~internal_action_name", "/teb_internal/move_base")
        self.map_frame = rospy.get_param("~map_frame", "map")
        self.real_base_frame = rospy.get_param("~real_base_frame", "base_footprint")
        self.virtual_base_frame = rospy.get_param("~virtual_base_frame", "teb_base_footprint")
        self.raw_cmd_topic = rospy.get_param("~cmd_vel_raw_topic", "/cmd_vel_teb_raw")
        ...
        self.display_relays = [
            self.make_relay(
                "/teb_internal/move_base/global_costmap/costmap",
                "/move_base/global_costmap/costmap",
                OccupancyGrid,
            ),
            self.make_relay(
                "/teb_internal/move_base/local_costmap/costmap",
                "/move_base/local_costmap/costmap",
                OccupancyGrid,
            ),
            self.make_relay(
                "/teb_internal/move_base/GlobalPlanner/plan",
    ...
    def start(self):
        ...
        rospy.loginfo("Virtual reverse adapter ready. Public action: /%s", self.external_action_name)
    ...
    def simple_goal_callback(self, pose_stamped):
        ...
        self.set_mode(mode)
        rospy.loginfo(
            "Simple goal mode: %s, forwarding to %s",
            mode,
    ...
    def select_mode(self, target_pose_stamped):
        try:
            ...
            # RViz goals are stamped at click time; using time 0 avoids false
            # extrapolation failures when TF lags the click by a few ms.
        ...
        yaw_ok = abs(normalize_angle(target_yaw - current_yaw)) <= self.reverse_yaw_tolerance

        if behind_ok and lateral_ok and yaw_ok:
            rospy.loginfo(
                "Reverse goal detected: x=%.2f y=%.2f dist=%.2f yaw_err=%.2f",
                x, y, distance, normalize_angle(target_yaw - current_yaw)
            )
...
if __name__ == "__main__":
    rospy.init_node("teb_virtual_reverse_adapter")
    adapter = TebVirtualReverseAdapter()
    adapter.start()
    rospy.spin()
